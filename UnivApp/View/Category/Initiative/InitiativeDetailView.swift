    ...
    var titleView: some View {
        HStack {
            VStack(alignment: .leading, spacing: 10) {
                ...
                Text("\(model.compactMap { $0.year }.first ?? "")년도")
                    .font(.system(size: 15, weight: .bold))
                    .foregroundColor(.black)
                    .padding(.top, 10)
            }
...
#Preview {
    InitiativeDetailView(model: [InitiativeModel(displayName: "", fullName: "세계대학평가", description: "어쩌고저쩌고", year: "", universityRankingResponses: [UniversityRankingResponses(universityName: "", logo: "", rank: 0)])], title: "CWTS")
}
